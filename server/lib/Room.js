...
class Room extends EventEmitter {
  ...
  handleProtooConnection({ peerId, consume, protooWebSocketTransport }) {
    ...
    peer
      .notify("mediasoup-version", { version: mediasoup.version })
      .catch(() => {});
      ...
      this._handleProtooRequest(peer, request, accept, reject)
        .then(async () => {
          ...
          if (request.method === "join") {
            ...
            // ✅ 2. 새로 들어온 peer가 기존 peer들의 미디어를 소비할 수 있도록 consumer 생성
            for (const otherPeer of this._getJoinedPeers({
              excludePeer: peer,
            })) {
              for (const producer of otherPeer.data.producers.values()) {
                await this._createConsumer({
                  consumerPeer: peer, // 새로 들어온 사람
                  producerPeer: otherPeer, // 기존의 사람들
                  producer,
                });
              }
            }
      ...
      case "createWebRtcTransport": {
        ...
        if (forceTcp) {
          webRtcTransportOptions.listenInfos =
            webRtcTransportOptions.listenInfos.filter(
